...
class tri_race /* szülő */
 {
  protected:
   ...
   protected:
    ...
    tri_race(string lc, int sw, int cyc, int rn, int dp) 
...
class in_ch : public tri_race, protected h_m_s    /* gyerek */
 {
  protected: 
   ...
   int i_j;
   vector<in_ch*> objs;
   in_ch(string lc, int sw, int cyc, int rn, int dp, int kt, int ij) : \
  ...
  public:
   ...
   void ch_objs(string lc, int sw, int cyc, int rn, int dp, int kt, int ij);
   void ch_objs(string lc, int sw, int cyc, int rn, int dp, int kt, int ij, char nm);
   void mind1() const ;
   void ossz_csap();
   ~in_ch();
 };
...
class in_gr_ch : public in_ch  /* unoka */
 {
  protected:
   ...
   in_gr_ch(string lc, int sw, int cyc, int rn, int dp, int kt, int ij, char nm)\
...
void in_ch::ch_objs(string lc, int sw, int cyc, int rn, int dp, int kt, int ij)
 {
  in_ch* n_c_o = new in_ch(lc, sw, cyc-ij, rn, dp, kt, ij);
  b_c_o(n_c_o);
 }

void in_ch::ch_objs(string lc, int sw, int cyc, int rn, int dp, int kt, int ij, char nm)
 {
...
in_ch::~in_ch()
 { 
  ...
  if(tagok.getLic() == "Epszilon") cout << "\nTöröltem a gyerek osztályra -> memóriát!";
 }
 
in_ch* in_ch::operator+(in_ch* m_v)
 {
  in_ch* csap = new in_ch; cout << ", a címe: " << csap;
...
void in_ch::ossz_csap()
 {
  //in_ch* csapat = *objs[0] + objs[1];
  in_ch* csapat = objs[0]->operator+(objs[1]);
...
int main(void)
 {
  in_ch alap;
  alap.ch_objs("Alfa",1200,4200,2300,123,1,0);
  alap.ch_objs("Beta",1300,4300,2400,125,2,0);
  alap.ch_objs("Gamma",1400,4400,2400,130,3,120);
  alap.ch_objs("Delta",1450,4400,2400,130,5,0);
  alap.ch_objs("Epszilon",1450,4400,2300,120,4,180,'F');
  alap.ossz_csap();
  alap.mind1();
  return 0;
}
